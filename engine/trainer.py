...
def part_trainer(model, optimizer, loss_fn, device=None):
    ...
    def _update(engine, batch):
        model.train()
        optimizer.zero_grad()
        img, target, mask1, mask2, mask3, mask4, mask5, mask6 = batch
        img = img.to(device) if torch.cuda.device_count() >= 1 else img
        target = target.to(device) if torch.cuda.device_count() >= 1 else target
        score, feat = model(img, mask1, mask2, mask3, mask4, mask5, mask6)
        # loss = loss_fn(score, feat, target)
        # loss.backward()
        # optimizer.step()
...
def do_train_part(
    ...
    RunningAverage(output_transform=lambda x: x[0][5]).attach(trainer, 'avg_loss')
    ...
    RunningAverage(output_transform=lambda x: x[1][5]).attach(trainer, 'avg_acc')
