...
def do_train_part(
    ...
    # average metric to attach on trainer
    for i in range(cfg.INPUT.PART):
        RunningAverage(output_transform=lambda x: x[0][i]).attach(trainer, 'avg_loss'+str(i+1))
        RunningAverage(output_transform=lambda x: x[1][i]).attach(trainer, 'avg_acc'+str(i+1))
